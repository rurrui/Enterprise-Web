using System;
using System.Data;
using System.Text;
using System.Data.SqlClient;
using Maticsoft.DBUtility;//Please add references
namespace SJD.DAL
{
	/// <summary>
	/// 数据访问类:Production
	/// </summary>
	public partial class Production
	{
		public Production()
		{}
		#region  BasicMethod

		/// <summary>
		/// 得到最大ID
		/// </summary>
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("ProId", "Production"); 
		}

		/// <summary>
		/// 是否存在该记录
		/// </summary>
		public bool Exists(int ProId)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from Production");
			strSql.Append(" where ProId=@ProId");
			SqlParameter[] parameters = {
					new SqlParameter("@ProId", SqlDbType.Int,4)
			};
			parameters[0].Value = ProId;

			return DbHelperSQL.Exists(strSql.ToString(),parameters);
		}


		/// <summary>
		/// 增加一条数据
		/// </summary>
		public int Add(SJD.Model.Production model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into Production(");
			strSql.Append("ProTitle,ProContent,ProPicSrc)");
			strSql.Append(" values (");
			strSql.Append("@ProTitle,@ProContent,@ProPicSrc)");
			strSql.Append(";select @@IDENTITY");
			SqlParameter[] parameters = {
					new SqlParameter("@ProTitle", SqlDbType.NVarChar,50),
					new SqlParameter("@ProContent", SqlDbType.Text),
					new SqlParameter("@ProPicSrc", SqlDbType.NVarChar,500)};
			parameters[0].Value = model.ProTitle;
			parameters[1].Value = model.ProContent;
			parameters[2].Value = model.ProPicSrc;

			object obj = DbHelperSQL.GetSingle(strSql.ToString(),parameters);
			if (obj == null)
			{
				return 0;
			}
			else
			{
				return Convert.ToInt32(obj);
			}
		}
		/// <summary>
		/// 更新一条数据
		/// </summary>
		public bool Update(SJD.Model.Production model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update Production set ");
			strSql.Append("ProTitle=@ProTitle,");
			strSql.Append("ProContent=@ProContent,");
			strSql.Append("ProPicSrc=@ProPicSrc");
			strSql.Append(" where ProId=@ProId");
			SqlParameter[] parameters = {
					new SqlParameter("@ProTitle", SqlDbType.NVarChar,50),
					new SqlParameter("@ProContent", SqlDbType.Text),
					new SqlParameter("@ProPicSrc", SqlDbType.NVarChar,500),
					new SqlParameter("@ProId", SqlDbType.Int,4)};
			parameters[0].Value = model.ProTitle;
			parameters[1].Value = model.ProContent;
			parameters[2].Value = model.ProPicSrc;
			parameters[3].Value = model.ProId;

			int rows=DbHelperSQL.ExecuteSql(strSql.ToString(),parameters);
			if (rows > 0)
			{
				return true;
			}
			else
			{
				return false;
			}
		}

		/// <summary>
		/// 删除一条数据
		/// </summary>
		public bool Delete(int ProId)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from Production ");
			strSql.Append(" where ProId=@ProId");
			SqlParameter[] parameters = {
					new SqlParameter("@ProId", SqlDbType.Int,4)
			};
			parameters[0].Value = ProId;

			int rows=DbHelperSQL.ExecuteSql(strSql.ToString(),parameters);
			if (rows > 0)
			{
				return true;
			}
			else
			{
				return false;
			}
		}
		/// <summary>
		/// 批量删除数据
		/// </summary>
		public bool DeleteList(string ProIdlist )
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from Production ");
			strSql.Append(" where ProId in ("+ProIdlist + ")  ");
			int rows=DbHelperSQL.ExecuteSql(strSql.ToString());
			if (rows > 0)
			{
				return true;
			}
			else
			{
				return false;
			}
		}


		/// <summary>
		/// 得到一个对象实体
		/// </summary>
		public SJD.Model.Production GetModel(int ProId)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ProId,ProTitle,ProContent,ProPicSrc from Production ");
			strSql.Append(" where ProId=@ProId");
			SqlParameter[] parameters = {
					new SqlParameter("@ProId", SqlDbType.Int,4)
			};
			parameters[0].Value = ProId;

			SJD.Model.Production model=new SJD.Model.Production();
			DataSet ds=DbHelperSQL.Query(strSql.ToString(),parameters);
			if(ds.Tables[0].Rows.Count>0)
			{
				return DataRowToModel(ds.Tables[0].Rows[0]);
			}
			else
			{
				return null;
			}
		}


		/// <summary>
		/// 得到一个对象实体
		/// </summary>
		public SJD.Model.Production DataRowToModel(DataRow row)
		{
			SJD.Model.Production model=new SJD.Model.Production();
			if (row != null)
			{
				if(row["ProId"]!=null && row["ProId"].ToString()!="")
				{
					model.ProId=int.Parse(row["ProId"].ToString());
				}
				if(row["ProTitle"]!=null)
				{
					model.ProTitle=row["ProTitle"].ToString();
				}
				if(row["ProContent"]!=null)
				{
					model.ProContent=row["ProContent"].ToString();
				}
				if(row["ProPicSrc"]!=null)
				{
					model.ProPicSrc=row["ProPicSrc"].ToString();
				}
			}
			return model;
		}

		/// <summary>
		/// 获得数据列表
		/// </summary>
		public DataSet GetList(string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ProId,ProTitle,ProContent,ProPicSrc ");
			strSql.Append(" FROM Production ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
			return DbHelperSQL.Query(strSql.ToString());
		}

		/// <summary>
		/// 获得前几行数据
		/// </summary>
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ");
			if(Top>0)
			{
				strSql.Append(" top "+Top.ToString());
			}
			strSql.Append(" ProId,ProTitle,ProContent,ProPicSrc ");
			strSql.Append(" FROM Production ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
			strSql.Append(" order by " + filedOrder);
			return DbHelperSQL.Query(strSql.ToString());
		}

		/// <summary>
		/// 获取记录总数
		/// </summary>
		public int GetRecordCount(string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM Production ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
			object obj = DbHelperSQL.GetSingle(strSql.ToString());
			if (obj == null)
			{
				return 0;
			}
			else
			{
				return Convert.ToInt32(obj);
			}
		}
		/// <summary>
		/// 分页获取数据列表
		/// </summary>
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("SELECT * FROM ( ");
			strSql.Append(" SELECT ROW_NUMBER() OVER (");
			if (!string.IsNullOrEmpty(orderby.Trim()))
			{
				strSql.Append("order by T." + orderby );
			}
			else
			{
				strSql.Append("order by T.ProId desc");
			}
			strSql.Append(")AS Row, T.*  from Production T ");
			if (!string.IsNullOrEmpty(strWhere.Trim()))
			{
				strSql.Append(" WHERE " + strWhere);
			}
			strSql.Append(" ) TT");
			strSql.AppendFormat(" WHERE TT.Row between {0} and {1}", startIndex, endIndex);
			return DbHelperSQL.Query(strSql.ToString());
		}

		/*
		/// <summary>
		/// 分页获取数据列表
		/// </summary>
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			SqlParameter[] parameters = {
					new SqlParameter("@tblName", SqlDbType.VarChar, 255),
					new SqlParameter("@fldName", SqlDbType.VarChar, 255),
					new SqlParameter("@PageSize", SqlDbType.Int),
					new SqlParameter("@PageIndex", SqlDbType.Int),
					new SqlParameter("@IsReCount", SqlDbType.Bit),
					new SqlParameter("@OrderType", SqlDbType.Bit),
					new SqlParameter("@strWhere", SqlDbType.VarChar,1000),
					};
			parameters[0].Value = "Production";
			parameters[1].Value = "ProId";
			parameters[2].Value = PageSize;
			parameters[3].Value = PageIndex;
			parameters[4].Value = 0;
			parameters[5].Value = 0;
			parameters[6].Value = strWhere;	
			return DbHelperSQL.RunProcedure("UP_GetRecordByPage",parameters,"ds");
		}*/

		#endregion  BasicMethod
		#region  ExtensionMethod

		#endregion  ExtensionMethod
	}
}

