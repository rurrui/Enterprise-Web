...
{
    public partial class super_recruit : MyBasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            int rowCount = reBll.GetRecordCount("");
            ...
            DataTable dt = reBll.GetListByPage("", "RecruitId", startIndex, endIndex).Tables[0];
            ...
            foreach (DataRow row in dt.Rows)
            {
                sb.AppendFormat("<tr><td style='text-align:center'>{0}</td>" +
                    ...
                    "<td style='text-align:center'><a href='super-edit-recruit.aspx?id={3}' class='btn btn-success'>修改</a></td>" +
                    "<td style='text-align:center'><a href='javascript:DeleteConfirm({4})' class='btn btn-danger'>删除</a></td></tr>", Convert.ToInt32(row["RecruitId"]), row["RecruitTitle"], Convert.ToDateTime(row["RecruitTime"]).ToString("yyyy-MM-dd"), Convert.ToInt32(row["RecruitId"]), Convert.ToInt32(row["RecruitId"]));
            }
            strHtml = sb.ToString();
        }
    }
